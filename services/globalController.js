const serviceModel = require('./globalModel'); 
...
class globalController {
    constructor(schemaFilter = [], model = {}, includedModel = []){
            ...
            this._Model = new serviceModel(model);
    ...
    get (req,res)  {
        ...
        return this._Model.find(headersValidation,this._IncludedModel)
        .then(data=>res.json({data,success:true}))
        .catch(err=>res.json(errMsg(err)))
    }
    update (req,res)  {
        ...
        return this._Model.find(bodyValidation)   
        .then(data=>{
    ...
    remove  (req,res) { // for force remove 
        ...
        return this._Model.remove(headersValidation)
